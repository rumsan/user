...
let { RoleSchema } = require("./schemas");
...
class Role {
  ...
  async listPermissions(name) {
    let role = await this.model.findOne({ name });
    return role.permissions;
  }

  async getValidRoles() {
    let roles = await this.model
      .find({
  ...
  async calculatePermissions(name) {
    ...
    let validRoles = await this.model.find({
      name: { $in: roles },
      $or: [{ expiry_date: null }, { expiry_date: { $gt: new Date() } }]
    });
