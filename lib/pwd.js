...
const { Error, ERR, throwError } = require("./error");
    ...
    return new Promise((resolve, reject) => {
      ...
      this.models.UserModel.findOne({ _id: user_id, is_active: true })
        .then(u => {
          ...
          user = u;
          return Utils.Secure.hash(oldPassword, Buffer.from(u.password.salt, "base64"));
        })
        .then(hashedOldPwd => {
          if (user.password.hash !== hashedOldPwd.hash.toString("base64")) throw ERR.PWD_NOTMATCH;
          return Utils.Secure.saltAndHash(newPassword);
        })
        .then(hashedNewPwd => {
          let password = {
      ...
      this.models.AuthModel.findOne({ username: email })
        ...
        .then(d => {
          ...
          let tkn = Math.floor(100000 + Math.random() * 900000).toString();
              ...
              this.messenger.send({to:d.username, data:{ name: d.user_id.name.full, token: tkn }, template: 'forgot'})
              .then(d => {
                this.models.AuthModel.findOne({ username : email})
                  .then(user => {
                    console.log(tkn,expiresIn,user.user_id);
                    return this.models.UserModel.findByIdAndUpdate({_id : user.user_id },{$set:{user_token : tkn, token_expiration : expiresIn }}, {new : true })
                  })
                })
