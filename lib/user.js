...
const {
  Secure, NameParser, ERR, TokenUtil, RoleUtil,
} = require('./utils');
...
class User {
  constructor(cfg) {
    ...
    this.model = cfg.model
      ...
      this.PasswordManager = new pwd({ models: this.model, messenger:cfg.messenger, smsService:cfg.smsService });
    }
  ...
  async authenticate({
    username, password, tokenData, loginBy = 'email',
  }) {
    if (!username) throw ERR.USERNAME_REQ;
    if (!password) throw ERR.PASSWORD_REQ;

    let user = null;
    if (loginBy === 'email') user = await this.model.findOne({ email: username }, '+password.hash +password.salt');
    if (loginBy === 'phone') user = await this.model.findOne({ phone: username }, '+password.hash +password.salt');
    if (!user) throw ERR.USER_NOEXISTS;
    const hashedPwd = await Secure.makeHash(password, Buffer.from(user.password.salt, 'base64'));
  ...
  async create(payload, options = {}) {
    ...
    if(user){
      ...
      if(notify==='email'){
        this.messenger.send({
          to: user.email,
          data: { name:user.name.full, email: user.email, password: rawPassword},
  ...
  update(id, payload, options = {}) {
    payload = { ...payload };
    if (payload.name && typeof payload.name === 'string') { payload.name = NameParser.parse(payload.name); }
