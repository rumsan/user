...
class User {
  ...
  constructor({ mongoose, messenger, appSecret, modelConfig, jwtDuration }) {
    ...
    this.PATManager = new pat({ models: this.models, messenger });
  ...
  addRoles({ user_id, roles }) {
    ...
    return UserModel.findByIdAndUpdate(user_id, { $addToSet: { roles } }, { new: 1 });
  }

  removeRole({ user_id, role }) {
    return UserModel.findByIdAndUpdate(user_id, { $pull: { roles: role } }, { new: 1 });
  }
