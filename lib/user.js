...
class User {
  ...
  async authenticate({
    ...
    let user = null;
    if (loginBy === 'email') user = await this.model.findOne({ email: username });
    if (loginBy === 'phone') user = await this.model.findOne({ phone: username });
  ...
  async getByEmail(email, { selectFields } = {}) {
    ...
    */
    try {
      selectFields = selectFields || '';
      console.log(email);
      const data = await this.model.findOne({ email: 'santosh@rumsan.com', is_active: true }, selectFields);
