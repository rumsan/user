...
const addCommsWithoutUserIdCheck = (user_id, comms) => {
  ...
  return new Promise((resolve, reject) => {
    CommModel.findOne({ where: { user_id } })
      .then((r) => {
        existingComms = r;
...
class User {
  /**
   * @db {object} Send reference to db from your application.
   ...
   */
  constructor({ db, messenger, appSecret, modelConfig, jwtDuration }) {
    if (!db) throw ERR.POSTGRES;

    // TODO: revert
    // if (!appSecret) throw ERR.APP_SECRET;
    // if (appSecret.length != 32) throw ERR.APP_SECRET32;

    this.models = buildModels({ db, modelConfig });
  ...
  async generatePassword(password) {
    ...
    let pwdHash = await Secure.saltAndHash(password);
    return {
      password,
      hash: pwdHash.hash.toString("base64"),
      salt: pwdHash.salt.toString("base64"),
  ...
  getById(
    user_id,
    { populateComms = true, returnPwd = false } = {
      populateComms: true,
      returnPwd: false,
    }
  ) {
    ...
    if (populateComms)
      ...
      return UserModel.findOne({ _id: user_id, is_active: true }, showFields);
  }
    ...
    if (!auth) {
      ...
      if (auth) {
        ...
        if (options.registerNewUser) {
          user = await this.registerByExternal({
            service,
            service_id,
            extras,
            options,
          ...
          throw new Error(
            "Please contact admin to give you access to the system first."
          );
      }
    }

    if (!user) user = await this.getById(auth.user_id);
    let token = await this.generateToken(user, tokenData, jwtDuration);
    user.token = token;
  ...
  async authenticatePAT({ key, secret }) {
    ...
    return UserModel.findOne(
      { _id: pat.user_id, is_active: true },
      { password: 0 }
    ).lean();
  }
  ...
  async updatePrimaryPhone(user_id, phone) {
    ...
    if (!comm.lastErrorObject.updatedExisting) {
      await UserModel.findByIdAndUpdate(
        user_id,
        { $addToSet: { comms: comm.value._id } },
        { new: true }
      );
    }
  ...
  verifyLogin(username, password, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.getByUsername(username, options.loginBy, { returnPwd: true })
        .then((user) =>
          Secure.hash(password, Buffer.from(user.password.salt, "base64")).then(
            (hashedPwd) => {
